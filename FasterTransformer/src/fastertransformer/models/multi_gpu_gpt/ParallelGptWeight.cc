    ...
    for (int l = 0; l < num_layer_; l++) {
        if (isValidLayerParallelId(l)) {
            decoder_layer_weights.push_back(new ParallelGptDecoderLayerWeight<T>(
                hidden_units_, inter_size_, tensor_para_size_, tensor_para_rank_, int8_mode_, gpt_variant_params_, tcp_agent_, l));
        }
        else {
...
ParallelGptWeight<T>::~ParallelGptWeight()
{
    if (is_maintain_buffer == true) {
        for (int i = 0; i < weights_ptr.size(); i++) {
            if(tcp_agent_ && weights_ptr[i]){
                try{
                    releaseSharedPtr((void*)(weights_ptr[i] - offsets[i]));
                }catch(const std::exception& e){
                    printf("At ParallelGptWeight::weight_ptr[%d] release %p:\n", i, (void*)(weights_ptr[i] - offsets[i]));
                    puts(e.what());
                }
            }else
    ...
    // printf("[%d %d] before release layer_weights_ptr\n", layer_para_rank_, tensor_para_rank_);
    for (int i = 0; i < num_layer_; i++) {
        // FT_LOG_INFO("[%d %d] layer %d:", layer_para_rank_, tensor_para_rank_ , i);
        delete decoder_layer_weights[i];
    }
    // printf("[%d %d] after release layer_weights_ptr\n", layer_para_rank_, tensor_para_rank_);
    ...
    // printf("after release tcp_agent_\n");
}
...
template<typename T>
const T* ParallelGptWeight<T>::get_position_encoding_table(){
    if(!position_encoding_table){
        offsets[0] = requestTensor(&weights_ptr[0], max_seq_len_ * hidden_units_, -2, 0, 1, tcp_agent_);
        position_encoding_table = weights_ptr[0];
    }
    return position_encoding_table;
}

template<typename T>
const T* ParallelGptWeight<T>::get_pre_decoder_embedding_table(){
    if(!pre_decoder_embedding_table){
        offsets[1] = requestTensor(&weights_ptr[1], vocab_size_ * hidden_units_, -2, 1, 1, tcp_agent_);
        pre_decoder_embedding_table = weights_ptr[1];
    }
    return pre_decoder_embedding_table;
}

template<typename T>
LayerNormWeight<T>& ParallelGptWeight<T>::get_post_decoder_layernorm(){
    if(!post_decoder_layernorm.beta){
        offsets[2] = requestTensor(&weights_ptr[2], hidden_units_, -1, 0, 1, tcp_agent_);
        post_decoder_layernorm.beta = weights_ptr[2];
    }
    if(!post_decoder_layernorm.gamma){
        offsets[3] = requestTensor(&weights_ptr[3], hidden_units_, -1, 1, 1, tcp_agent_);
        post_decoder_layernorm.gamma = weights_ptr[3];
    }
    return post_decoder_layernorm;
}

template<typename T>
DenseWeight<T>& ParallelGptWeight<T>::get_post_decoder_embedding(){
    if(!post_decoder_embedding.kernel){
        offsets[4] = requestTensor(&weights_ptr[4], hidden_units_ * vocab_size_, -1, 2, 1, tcp_agent_);
        post_decoder_embedding.kernel = weights_ptr[4];
    }
    return post_decoder_embedding;
}
...
void ParallelGptWeight<T>::mallocWeights()
{
    ...
    offsets.resize(num_base_weights + prompt_learning_pair_.size());

    if(tcp_agent_){
        /*
        offsets[0] = requestTensor(&weights_ptr[0], max_seq_len_ * hidden_units_, -1, 0, 1, tcp_agent_);
        offsets[1] = requestTensor(&weights_ptr[1], vocab_size_ * hidden_units_, -1, 1, 1, tcp_agent_);
        offsets[2] = requestTensor(&weights_ptr[2], hidden_units_, -1, 2, 1, tcp_agent_);
        offsets[3] = requestTensor(&weights_ptr[3], hidden_units_, -1, 3, 1, tcp_agent_);
        offsets[4] = requestTensor(&weights_ptr[4], hidden_units_ * vocab_size_, -1, 4, 1, tcp_agent_);
        */
    }else{
    ...
    if (malloc_load_prompt_weights_) {
        FT_CHECK_WITH_INFO(tcp_agent_ == NULL, "prompt learning with param clients has not supported yet.");
