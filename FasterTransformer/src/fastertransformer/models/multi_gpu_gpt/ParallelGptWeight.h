...
public:
    const T* get_position_encoding_table();
    const T* get_pre_decoder_embedding_table();
    LayerNormWeight<T>& get_post_decoder_layernorm();
    DenseWeight<T>& get_post_decoder_embedding();
    /*
    ...
    */
    ...
    TcpAgent* get_tcp_agent() const {return tcp_agent_; }
...
private:
    ...
    std::vector<int64_t> offsets       = std::vector<int64_t>(num_base_weights, 0);
