...
public:
    LayerNormWeight<T>& get_pre_layernorm_weights();
    AttentionWeight<T>& get_self_attention_weights();
    LayerNormWeight<T>& get_self_attn_layernorm_weights();
    FfnWeight<T>& get_ffn_weights();
...
private:
    void setWeightPtr();
    void mallocWeights(int layer_id = -1);
...
protected:
    ...
    std::vector<int64_t> offsets = std::vector<int64_t>(20, 0);
