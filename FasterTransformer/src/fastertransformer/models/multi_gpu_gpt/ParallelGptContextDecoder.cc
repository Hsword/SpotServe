...
template<typename T>
void ParallelGptContextDecoder<T>::initialize(bool tp_comm)
{
...
template<typename T>
bool ParallelGptContextDecoder<T>::isRealLayerId(uint l)
{
    ...
    remove_padding_(remove_padding)
{
    ...
    // FT_LOG_INFO("real layer %ld", real_num_layer_);
    initialize(tp_comm);
}
    ...
    /*const*/ std::vector<ParallelGptDecoderLayerWeight<T>*>* gpt_decoder_layer_weight)
{
    ...
    for (uint ite = 0; ite < iteration_num; ite++) {
        ...
        for (uint l = 0; l < num_layer_; l++) {
            ...
            // sync_and_profile(fmtstr("context_decoder_%d begin", l), 0);
            ...
            if(isRealLayerId(l)){
                ...
                FT_LOG_DEBUG("Context: Invalid PP layer %ld", l);
            }
            ...
            if ((l == last_layer_id) && remove_padding_) {
            ...
            // sync_and_profile(fmtstr("context_decoder_%d end", l), 0);
        }
    }
