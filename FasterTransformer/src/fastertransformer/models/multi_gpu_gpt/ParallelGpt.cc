...
template<typename T>
void ParallelGpt<T>::initialize(bool tp_comm)
{
...
void ParallelGpt<T>::allocateBuffer(size_t batch_size,
                                    ...
                                    const bool load_not_alloc)
{
    // const int param_type_template_mask = 0x6;
    float* combined_buffer_pointer = nullptr;
    int dp_buffer_op_type = load_not_alloc ? 3 : 7;
    ...
    if(tcp_agent_){
        requestTensor(&decoder_input_buf_, batchxbeam * hidden_units_, -2, 0, dp_buffer_op_type, tcp_agent_, is_allocate_buffer_);
    }else{
    ...
    if(tcp_agent_){
        // todo different type
        combined_buffer_pointer = reinterpret_cast<float*>(finished_buf_);
        requestTensor(&combined_buffer_pointer, batchxbeam * 4, -2, 1, dp_buffer_op_type, tcp_agent_, is_allocate_buffer_);
        // combined:
        // (bool)  finished_buf_     None       None        None
        // (float)                    cum_log_probs_
        // (int)                    sequence_lengths_
        // (int)                tiled_total_padding_count_
        finished_buf_               = reinterpret_cast<bool*>(combined_buffer_pointer);
        cum_log_probs_              = reinterpret_cast<float*>(combined_buffer_pointer + batchxbeam * 1);
        sequence_lengths_           = reinterpret_cast<int*>(combined_buffer_pointer + batchxbeam * 2);
        // print_to_screen(sequence_lengths_, batchxbeam);
        tiled_total_padding_count_  = reinterpret_cast<int*>(combined_buffer_pointer + batchxbeam * 3);
        // print_to_screen(tiled_total_padding_count_, batchxbeam);
    }else{
    ...
    if(tcp_agent_){
        int buffer_op_type = load_not_alloc ? 2 : 6;
        requestTensor(&key_cache_, self_cache_size, -3, 0, buffer_op_type, tcp_agent_, is_allocate_buffer_);
        requestTensor(&value_cache_, self_cache_size, -3, 1, buffer_op_type, tcp_agent_, is_allocate_buffer_);
    }else{
    ...
    if(tcp_agent_){
        float* temp = reinterpret_cast<float*>(output_ids_buf_);
        requestTensor(&temp, batchxbeam * max_session_len, -2, 2, dp_buffer_op_type, tcp_agent_, is_allocate_buffer_);
        output_ids_buf_ = reinterpret_cast<int*>(temp);
        temp = reinterpret_cast<float*>(parent_ids_buf_);
        requestTensor(&temp, batchxbeam * max_session_len, -2, 3, dp_buffer_op_type, tcp_agent_, is_allocate_buffer_);
        parent_ids_buf_ = reinterpret_cast<int*>(temp);
    }else{
    ...
    if(tcp_agent_){
        requestTensor(&output_log_probs_buf_, batchxbeam * max_session_len, -2, 4, dp_buffer_op_type, tcp_agent_, is_allocate_buffer_);
    }else{
    ...
    if (beam_width > 1) {
        if(tcp_agent_){
            float* temp = reinterpret_cast<float*>(cache_indirections_[0]);
            requestTensor(&temp, batchxbeam * memory_len * 2, -2, 5, dp_buffer_op_type, tcp_agent_, is_allocate_buffer_);
            cache_indirections_[0] = reinterpret_cast<int*>(temp);
        }else{
...
void ParallelGpt<T>::freeBuffer()
{
    if (is_allocate_buffer_) {
        ...
        if(tcp_agent_){
            releaseSharedPtr((void*)decoder_input_buf_);
            decoder_input_buf_ = nullptr;
            releaseSharedPtr((void*)finished_buf_);
            finished_buf_ = nullptr;
            cum_log_probs_ = nullptr;
            sequence_lengths_ = nullptr;
            tiled_total_padding_count_ = nullptr;
            releaseSharedPtr((void*)key_cache_);
            key_cache_ = nullptr;
            releaseSharedPtr((void*)value_cache_);
            value_cache_ = nullptr;
            releaseSharedPtr((void*)output_ids_buf_);
            output_ids_buf_ = nullptr;
            releaseSharedPtr((void*)parent_ids_buf_);
            parent_ids_buf_ = nullptr;
            releaseSharedPtr((void*)output_log_probs_buf_);
            output_log_probs_buf_ = nullptr;
            if (cache_indirections_[0] != nullptr) {
                releaseSharedPtr((void*)cache_indirections_[0]);
...
{
    real_num_layer_ = real_layer_num == -1 ? num_layer : real_layer_num;
    // FT_LOG_INFO("gpt.cc: real layer %ld", real_num_layer_);
...
ParallelGpt<T>::ParallelGpt(ParallelGpt<T> const& gpt):
    ...
    tcp_agent_(gpt.tcp_agent_),
    real_num_layer_(gpt.real_num_layer_)
{
    initialize();
}
                                               ...
                                               /*const*/ ParallelGptWeight<T>* gpt_weights)
{
    ...
    if (pipeline_para_.rank_ == pipeline_para_.world_size_ - 1) {
        ...
        invokeGeneralLayerNorm(lp_normed_decoder_output_buf_,
                               ...
                               gpt_weights->get_post_decoder_layernorm().gamma,
                               gpt_weights->get_post_decoder_layernorm().beta,
                               layernorm_eps_,
                               n_hidden_states,
                               hidden_units_,
                               stream_);
        sync_check_cuda_error();
        init_padded_embedding_kernel_ptr(gpt_weights);
...
template<typename T>
void ParallelGpt<T>::init_padded_embedding_kernel_ptr(ParallelGptWeight<T>* gpt_weights){
    if(padded_embedding_kernel_ptr_inited_) return;
    padded_embedding_kernel_ptr_inited_ = true;
    if (vocab_size_ == vocab_size_padded_) {
        padded_embedding_kernel_ptr_ = gpt_weights->get_post_decoder_embedding().kernel;
    }
    else {
        cudaAutoCpy(padded_embedding_kernel_,
                    gpt_weights->get_post_decoder_embedding().kernel,
                             ...
                             /*const*/ ParallelGptWeight<T>*                    gpt_weights)
{
    ...
    //      NEW start_step [1] on cpu, int32, optional
    //      NEW iter_step [1] on cpu, int32, optional
    ...
    bool continue_gen = input_tensors->find("continue_gen") != input_tensors->end() ?
                            ...
                            false;
    int start_step = input_tensors->find("start_step") != input_tensors->end() ? 
                        input_tensors->at("start_step").getVal<int>() : 0;
    const bool restore_buffer_from_pc = start_step != 0;
    ...
    size_t gen_len          = input_tensors->at("output_seq_len").max<uint32_t>() + limit_len_offset;
    ...
    if(input_tensors->find("iter_step") != input_tensors->end()){
        // for restoring
        size_t new_gen_len = max_input_length + start_step + input_tensors->at("iter_step").getVal<int>();
        if(new_gen_len < gen_len){
            will_finish = false;
            gen_len = new_gen_len;
        }
    }
    ...
    if (!continue_gen) {
        allocateBuffer(batch_size,
                       ...
                       restore_buffer_from_pc);
        padded_embedding_kernel_ptr_inited_ = false;
        sync_check_cuda_error();
    }
    ...
    if (continue_gen) {
        ...
        if (has_p_prompt_tuning_ || has_prefix_prompt_ || has_prefix_soft_prompt_ || max_input_length > 1) {
            ...
            sync_check_cuda_error();
            if(!restore_buffer_from_pc){
                // only when not restoring
                // sync_and_profile("first_embedding before", 0);
                if (has_prefix_soft_prompt_) {
                    ...
                    param.embedding_table               = gpt_weights->get_pre_decoder_embedding_table();
                    param.pos_table                     = gpt_weights->get_position_encoding_table();
                    ...
                    invokeInputIdsEmbeddingLookupPosEncoding(context_decoder_input_buf_,
                                                            output_ids_buf_,
                                                            gpt_weights->get_pre_decoder_embedding_table(),
                                                            gpt_weights->get_position_encoding_table(),
                                                            prompt_param,
                                                            tiled_input_ids_buf_,
                                                            1,
                                                            max_input_length,
                ...
                invokeBuildDecoderAttentionMask(input_attention_mask_,
                                                ...
                                                stream_);
                sync_check_cuda_error();
                // sync_and_profile("first_embedding after", 0);
                ...
                // sync_and_profile("call gpt_context_decoder_forward", 0);
                gpt_context_decoder_->forward(
                    &decoder_output_tensors, &decoder_input_tensors, &gpt_weights->decoder_layer_weights);
                // sync_and_profile("call gpt_context_decoder_forward returned", 0);
    ...
    for (step_ = restore_buffer_from_pc ? start_step : step_start; step_ < (int)gen_len; step_++) {
        ...
        *generation_should_stop_   = !fill_caches_only;
        ...
        for (uint ite = 0; ite < iteration_num; ++ite) {
            ...
            // FT_LOG_INFO("Iter: step_ = %d, ite = %d", step_, ite);
            ...
            if ((max_input_length <= 1) || (step_ > step_start) || continue_gen) {
                ...
                std::vector<Tensor> decoder_output_tensors{
                    ...
                    Tensor{MEMORY_GPU, data_type, self_v_cache_shape, value_cache_}};
                // if (step_ == 16 && ite == 0){
                //     // print_to_file(decoder_input_buf_, local_batch_size * beam_width * hidden_units_, fmtstr("c%dr%d_decoder_input_buf.pdata", restore_buffer_from_pc, tensor_para_.rank_).c_str());
                //     // print_to_file(finished_buf_, local_batch_size * beam_width, fmtstr("c%dr%d_finished_buf.pdata", restore_buffer_from_pc, tensor_para_.rank_).c_str());
                //     // print_to_file(sequence_lengths_, local_batch_size * beam_width, fmtstr("c%dr%d_sequence_lengths.pdata", restore_buffer_from_pc, tensor_para_.rank_).c_str());
                //     // print_to_file(tiled_total_padding_count_, local_batch_size * beam_width, fmtstr("c%dr%d_tiled_total_padding_count.pdata", restore_buffer_from_pc, tensor_para_.rank_).c_str());
                //     // printf("max_context_len=%d step_=%d, ite=%u\n", max_context_len, step_, ite);
                //     // print_to_file(masked_tokens_, local_batch_size * beam_width * memory_len, fmtstr("c%dr%d_masked_tokens.pdata", restore_buffer_from_pc, tensor_para_.rank_).c_str());
                //     // print_to_file(output_ids_buf_, local_batch_size * beam_width, fmtstr("c%dr%d_output_ids_buf_.pdata", restore_buffer_from_pc, tensor_para_.rank_).c_str());
                //     print_to_file(key_cache_, num_layer_ / pipeline_para_.world_size_ * batch_size * beam_width * local_head_num_ * memory_len * size_per_head_, fmtstr("c%dr%d_key_cache_.pdata", restore_buffer_from_pc, tensor_para_.rank_).c_str());
                //     print_to_file(value_cache_, num_layer_ / pipeline_para_.world_size_ * batch_size * beam_width * local_head_num_ * memory_len * size_per_head_, fmtstr("c%dr%d_value_cache_.pdata", restore_buffer_from_pc, tensor_para_.rank_).c_str());
                // }
                
                // sync_and_profile(fmtstr("call gpt_decoder_forward_s%d before", step_), 0);
                gpt_decoder_->forward(
                    &decoder_output_tensors, &decoder_input_tensors, &gpt_weights->decoder_layer_weights);
                // sync_and_profile(fmtstr("call gpt_decoder_forward_s%d end", step_), 0);
            }
            ...
            if (!fill_caches_only && pipeline_para_.rank_ == pipeline_para_.world_size_ - 1) {
                ...
                if (tensor_para_.world_size_ == 1) {
                    ...
                    // sync_and_profile(fmtstr("final embedding %d end", step_), 0);
                }
                else {
                    FT_CHECK(vocab_size_padded_ % tensor_para_.world_size_ == 0);
                    const int local_vocab_size = vocab_size_padded_ / tensor_para_.world_size_;
                    ...
                    ftNcclAllGather(nccl_logits_buf_ + vocab_size_units_offset,
                                    ...
                                    tensor_para_,
                                    stream_);
                    // sync_and_profile(fmtstr("final all gather %d end", step_), 0);
                ...
                // sync_and_profile(fmtstr("call gpt_decoder_dynamic_forward_s%d before", step_), 0);
                dynamic_decode_layer_->forward(&dynamic_decode_output_tensors, &dynamic_decode_input_tensors);
                *generation_should_stop_ &= subbatch_should_stop;
                // sync_and_profile(fmtstr("call gpt_decoder_dynamic_forward_s%d end", step_), 0);
            }
        }
...
template<typename T>
void ParallelGpt<T>::blockUntilAllParamReady(){
    T* ptr;
    requestTensor(&ptr, hidden_units_, -1, 0, 9, tcp_agent_);
    return;
}
