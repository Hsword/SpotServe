...
void allReduce(void* sendbuf, void* recvbuf, size_t size, MpiType dtype, MpiOp op, MpiComm comm)
{
#ifdef BUILD_MULTI_GPU
    MPICHECK(MPI_Allreduce(sendbuf, recvbuf, size, getMpiDtype(dtype), getMpiOp(op), comm.group));
#endif
}
