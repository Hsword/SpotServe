...
void ftNcclInitializeWithCoordinate(NcclParam& tensor_para,
                                    NcclParam& pipeline_para,
                                    const int  tensor_para_size,
                                    const int  pipeline_para_size,
                                    const int  tensor_para_rank,
                                    const int  pipeline_para_rank)
{
    // Initialize nccl communication grid of tensor and pipeline parallel groups.
#ifndef BUILD_MULTI_GPU
    FT_CHECK_WITH_INFO(tensor_para_size == 1,
    ...
    if (tensor_para_rank == 0) {
        FT_LOG_DEBUG("rank %d pp rank %d creates nccl uid.", rank, tensor_para_rank);
        NCCLCHECK(ncclGetUniqueId(&tp_uid));
    }
    if (pipeline_para_rank == 0) {
        FT_LOG_DEBUG("rank %d pp rank %d creates nccl uid.", rank, pipeline_para_rank);
        NCCLCHECK(ncclGetUniqueId(&pp_uid));
    }
    // Broadcast nccl uid to share the same nccl uid across gpus in the same group.
    FT_LOG_DEBUG("Broadcast nccl uid to the others in the same parallel groups.");
    std::vector<ncclUniqueId> comm_uids(tensor_para_size + pipeline_para_size);
    memset(comm_uids.data(), 0, sizeof(ncclUniqueId) * comm_uids.size());
    if (tensor_para_rank == 0) {
        memcpy(comm_uids.data() + pipeline_para_rank, &tp_uid, sizeof(ncclUniqueId));
    }
    if (pipeline_para_rank == 0) {
        memcpy(comm_uids.data() + pipeline_para_size + tensor_para_rank, &pp_uid, sizeof(ncclUniqueId));
    }
    MPI_Allreduce(MPI_IN_PLACE, comm_uids.data(), sizeof(ncclUniqueId) * comm_uids.size(), MPI_BYTE, MPI_SUM,
                  MPI_COMM_WORLD);

    if (tensor_para_rank != 0) {
        memcpy(&tp_uid, comm_uids.data() + pipeline_para_rank, sizeof(ncclUniqueId));
    }
    if (pipeline_para_rank != 0) {
        memcpy(&pp_uid, comm_uids.data() + pipeline_para_size + tensor_para_rank, sizeof(ncclUniqueId));
    }

    FT_LOG_DEBUG("Initialize NCCL communicators.");
    ncclComm_t tp_nccl_comm, pp_nccl_comm;
    NCCLCHECK(ncclCommInitRank(&tp_nccl_comm, tensor_para_size, tp_uid, tensor_para_rank));
    NCCLCHECK(ncclCommInitRank(&pp_nccl_comm, pipeline_para_size, pp_uid, pipeline_para_rank));

    tensor_para.world_size_   = tensor_para_size;
    tensor_para.rank_         = tensor_para_rank;
    tensor_para.nccl_uid_     = tp_uid;
    tensor_para.nccl_comm_    = tp_nccl_comm;
    pipeline_para.world_size_ = pipeline_para_size;
    pipeline_para.rank_       = pipeline_para_rank;
    pipeline_para.nccl_uid_   = pp_uid;
    pipeline_para.nccl_comm_  = pp_nccl_comm;
    FT_LOG_INFO("NCCL initialized rank=%d world_size=%d tensor_para=%s pipeline_para=%s",
                rank,
...
size_t getLocalBatchSize(const size_t batch_size, const size_t seq_len, const size_t pipeline_para_size) {
    ...
    if (BSZ > 0) {
        if (seq_len > 1) {
            if (batch_size % M1 == 0) {
                return batch_size / M1;
            } else {
                return batch_size;
            }
        } else {
            if (batch_size % M2 == 0) {
                return batch_size / M2;
            } else {
                return batch_size;
            }
        }
