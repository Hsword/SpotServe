                                                        ...
                                                        const AttentionWeight<T>*                     attention_weights)
{
    ...
    // sync_and_profile("GptContextAttentionLayer::forward before", 0);
    GptContextAttentionLayer<T>::forward(output_tensors, input_tensors, attention_weights);
    // sync_and_profile("GptContextAttentionLayer::forward end", 0);
    ...
    if (do_all_reduce_ && tensor_para_.world_size_ > 1) {
        ...
        // sync_and_profile("GptContextAttentionLaye::ftNcclAllReduceSum end", 0);
    }
}
