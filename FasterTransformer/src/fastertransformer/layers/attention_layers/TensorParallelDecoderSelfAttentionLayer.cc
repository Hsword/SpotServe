                                                         ...
                                                         const AttentionWeight<T>* attention_weights)
{
    ...
    // sync_and_profile("DecoderSelfAttentionLayer::forward before", 0);
    DecoderSelfAttentionLayer<T>::forward(output_tensors, input_tensors, attention_weights);
    // sync_and_profile("DecoderSelfAttentionLayer::forward end", 0);
    ...
    if (tensor_para_.world_size_ > 1 && do_all_reduce_) {
        ...
        // sync_and_profile("DecoderSelfAttentionLayer::ftNcclAllReduceSum end", 0);
    }
}
