...
template<typename T>
void multi_gpu_gpt_example(const INIReader reader, std::string in_csv, int mini_bsz, int tp_deg, int pp_deg, int start_step=0, int iter_step=0);
...
int main(int argc, char* argv[])
{
    ...
    int start_step = 0, iter_step = 0;
    if(argc >= 2) mini_bsz = atoi(argv[1]);
    if(argc >= 3) tp_deg = atoi(argv[2]);
    if(argc >= 4) pp_deg = atoi(argv[3]);
    if(argc >= 5) start_step = atoi(argv[4]);
    if(argc >= 6) iter_step = atoi(argv[5]);

    std::string ini_name;
    if (argc >= 7) {
        ini_name = std::string(argv[6]);
    }
    else {
    ...
    if (data_type == "fp32") {
        multi_gpu_gpt_example<float>(reader, in_csv, mini_bsz, tp_deg, pp_deg, start_step, iter_step);
    }
    else if (data_type == "fp16") {
        multi_gpu_gpt_example<half>(reader, in_csv, mini_bsz, tp_deg, pp_deg, start_step, iter_step);
    }
#ifdef ENABLE_BF16
...
template<typename T>
void multi_gpu_gpt_example(const INIReader reader, std::string in_csv, int mini_bsz, int tp_deg, int pp_deg, int start_step, int iter_step)
{
    ...
    for(int i = 0; i < ite; i++){
        ...
        max_input_len = max_input_lens[i];
        ...
        if (max_input_len == 0) {
            ...
            cudaH2Dcpy(d_input_ids, v_start_ids_list[i].data(), request_batch_size * max_input_len);
            cudaH2Dcpy(d_input_lengths, v_start_lengths_list[i].data(), request_batch_size);
        }
        ...
        if (start_step > 0){
            input_tensors_iter.insert({"start_step", {MEMORY_CPU, TYPE_INT32, {1}, &start_step}});
            printf("---inference continued at step %d---\n", start_step);
        }
        if (iter_step > 0){
            input_tensors_iter.insert({"iter_step", {MEMORY_CPU, TYPE_INT32, {1}, &iter_step}});
        }
        ...
        if(iter_step != 0){
            printf("---inference interupted at step %d---\n", gpt.get_step());
        }
    }
