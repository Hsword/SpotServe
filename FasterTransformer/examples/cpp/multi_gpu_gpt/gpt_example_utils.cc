                   ...
                   std::string       file_name)
{
    ...
    if (start_id_file.is_open()) {
        ...
        while (std::getline(start_id_file, line)) {
            ...
            if (i0 >= batch_size) break; // modified: only read 'batch_size' line(s)
        }
    }
    else {
    ...
    if (start_id_file.is_open()) {
        ...
        while (std::getline(start_id_file, line)) {
            ...
            if (i0 == batch_size){
                ...
                int max_input_len = tmp_start_lengths.data()[0];
                for (uint i = 1; i < (uint)tmp_start_lengths.size(); i++) {
                    max_input_len = max_input_len > tmp_start_lengths.data()[i] ? max_input_len : tmp_start_lengths.data()[i];
                }
                max_input_lens->push_back(max_input_len);
                ...
                std::vector<int> v_start_ids, v_start_lengths;
                for (int i = 0; i < (int)tmp_start_ids.size(); i++) {
                    for (int b = 0; b < beam_width; b++) {
                        for (int j = 0; j < (int)tmp_start_ids[i].size(); j++) {
                            v_start_ids.push_back(tmp_start_ids[i][j]);
                ...
                v_start_ids_list->push_back(v_start_ids);
                v_start_lengths_list->push_back(v_start_lengths);

                tmp_start_ids.clear();
                tmp_start_lengths.clear();
            }
        } // main while loop end
