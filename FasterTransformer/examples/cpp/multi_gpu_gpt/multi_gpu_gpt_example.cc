...
void multi_gpu_gpt_example(const INIReader reader, std::string in_csv)
{
    ...
    FT_LOG_INFO(rank, "NCCL init end");
    ...
    ParallelGptWeight<T> gpt_weights(hidden_units,
                                     ...
                                     gpt_variant_params);
    FT_LOG_INFO(rank, "load weight before");
    gpt_weights.loadModel(model_dir);
    FT_LOG_INFO(rank, "load weight end");
#ifdef SPARSITY_ENABLED
    if (sparse) {
    ...
    FT_LOG_INFO(rank, "load model end");
    ...
    cudaProfilerStart();
    FT_LOG_INFO(rank, "warmup before");
    // warm up
    ite = 1;
