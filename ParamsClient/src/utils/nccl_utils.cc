...
// modified
void ftNcclInitialize(NcclParam& nccl_para, const int para_size, const int rank){
    if (nccl_para.nccl_comm_ != nullptr) {
        FT_LOG_WARNING("NcclParam is already initialized. Skip NCCL initialization.");
        return;
    }
    FT_CHECK(nccl_para.nccl_comm_ == nullptr);
    FT_CHECK(para_size > 0);

    if (para_size == 1) {
        FT_LOG_WARNING("Skip NCCL initialization since requested parallel sizes are equals to 1.");
        nccl_para.rank_         = 0;
        nccl_para.world_size_   = para_size;
        return;
    }
    ...
    FT_LOG_DEBUG("Broadcast nccl uid to the others in the groups.");
    MPI_Bcast(&nccl_uid, sizeof(nccl_uid), MPI_BYTE, 0, MPI_COMM_WORLD);

    int nccl_rank = rank/* <= 7 ? rank * 2 : rank * 2 - 15*/;

    FT_LOG_DEBUG("Initialize NCCL communicators.");
    ncclComm_t nccl_comm;
    NCCLCHECK(ncclCommInitRank(&nccl_comm, para_size, nccl_uid, nccl_rank));

    nccl_para.world_size_   = para_size;
    nccl_para.rank_         = nccl_rank;
    nccl_para.nccl_uid_     = nccl_uid;
    nccl_para.nccl_comm_    = nccl_comm;   
}
